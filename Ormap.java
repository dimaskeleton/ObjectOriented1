...
// This class implements the IList<T> interface and represents a non-empty list
...
// This class implements the IList<T> interface and represents an empty list
...
class Course {
    ...
    */
    ...
    // method that calculates and returns the length of the deepest path in the course prerequisite history. Calls the accpet method with a visitor to calculate the length.
    int getDeepestPathLength() {
        return this.accept(new DeepestPathLength());
    }
    // method that checks whether a course has a prerequisite with a specified name. It calls the accept method with the HasPrereq visitor to check for the exsistence of a speicified prereq course
    boolean hasPrereq(String prereqName) {
        return this.accept(new HasPrereq(prereqName));
    }
}

// Represents a visitor that computes the deepest path length
class DeepestPathLength implements IListVisitor<Course, Integer> {
    ...
    */
    ...
    public Integer visitCons(Cons<Course> cons) {
        ...
        int depth = 1 + cons.first.getDeepestPathLength(); // Assuming each course contributes 1 to the depth
    ...
    // Since an empty list has no courses, its depth is 0, so return 0.
...
class HasPrereq implements IListVisitor<Course, Boolean> {
    String prereqName; // The name of the prerequisite course being checked for
    // Constructor
    ...
    */
    ...
    // We made a helper class to check if a single course has the specified prerequisite
    // The apply method of this class takes a Course as input and checks whether the course's name matches the specified prerequisite name or if the course itself has a prerequisite with the specified name
    private class CourseHasPrereq implements IFunc<Course, Boolean> {
        public Boolean apply(Course course) {
            return course.name.equals(prereqName) || course.hasPrereq(prereqName);
        }
    }

    // Method to visit a non-empty list of courses to check if any course in the list has the specified prerequisite
    public Boolean visitCons(Cons<Course> cons) {
        // We are using Ormap here to check if any course in the list has the prerequisite
        Ormap<Course> ormap = new Ormap<>(new CourseHasPrereq());
        return ormap.apply(cons) || cons.rest.accept(this);
    }

    // Method to visit an empty list of courses
    public Boolean visitMtList(MtList<Course> mt) {
        // Since an empty list contains no courses, it always returns false to show that no course with the specified prerequisite name was found.
        return false;
    }

    // Method to apply HasPrereq to a list of courses
    public Boolean apply(IList<Course> arg) {
        // Assign the operation to the list's accept method
        return arg.accept(this);
    }
}

// This Ormap class implements the IListVisitor interface to check if any element in a list satisfies a given predicate
class Ormap<T> implements IListVisitor<T, Boolean> {
    IFunc<T, Boolean> predicate;    //The predicate to apply to each element
    // Constructor
    Ormap(IFunc<T, Boolean> predicate) {
        this.predicate = predicate;
    }
    // Method to visit a non-empty list of elements
    public Boolean visitCons(Cons<T> cons) {
        // Apply the predicate to the first element
        // If true, return true without checking the rest of the list
        // Otherwise, recursively apply to the rest of the list
        return predicate.apply(cons.first) || cons.rest.accept(this);
    }
    // Method to visit an empty list of elements
    public Boolean visitMtList(MtList<T> mt) {
        // An empty list always returns false because there's nothing to satisfy the predicate
        return false;
    }
    // Method to apply Ormap to a list of elements
    public Boolean apply(IList<T> arg) {
        // Pass the operation to the list's accept method
        return arg.accept(this);
    }
}

// Examples class for the course.java code. Include sample instances of courses and tests for each method.
class ExamplesCourse {
    ExamplesCourse() {}

    // example courses a person can take
    ...
    //calculates the deepest path length for each course
    ...
    //Determine if a course has a specific prerequisite by comparing it with the provided prerequisite name
